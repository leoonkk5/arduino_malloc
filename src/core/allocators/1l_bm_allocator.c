...
each bit cooralates to the block in heap at : [(idx * 8 + bitpos) * BLOCK_SIZE]
*/
...
{   
    size_t prefix    = 0;   // Consecutive free blocks carried from previous bytes
    ...
    {   
        ...
        prefix += clzb(current);    // Add leading zeros (consecutive free blocks at the end of this byte) to prefix
...
/*
    Set a sequence of bits of lenght `count`, starting at index `idx` in the bitmap, 
...
static void _set_sequence(size_t idx, size_t count, uint8_t value)
{
    while (count > 0)
    {
        ...
        if (bits_in_byte > count) bits_in_byte = count;  // Limit to remaining count
...
static void *_1l_bm_allocate(size_t size) 
{
    ...
    // Fail immediatly if allocation is bigger than heap.
